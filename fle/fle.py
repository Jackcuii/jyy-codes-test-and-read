...
def FLE_exec(files):
    ...
    mem = mmap.mmap(         #直接把.load节用mmap syscall映射到进程地址空间 , and set the proper privilege
        fileno=-1, length=len(bs),
        prot=mmap.PROT_READ | mmap.PROT_WRITE | mmap.PROT_EXEC, # 这里甚至不分段
    ...
    func_type(func_ptr.value)()  # Not expect a return  跳转到程序入口_start执行
    exit(0)
    ...
    for iter in range(2):
        ...
        for sec_name, sec in sections.items():
            for fle, sec_body in sec:
                ...
                for item in sec_body: # 先循环一次，进行符号解析，合并同类项，符号都放到符号表里
                    match item.split(': '):
                        case '📤', symb:
...
def elf_to_fle(binary, section): #用GCC编译出来的ELF，直接解析翻译成FLE
    '''
...
def FLE_cc(options):
    ...
    #先用GCC，再OBJDUMP，再给每个段调用ELF_TO_FLE，纯真的字符串解析。
