...
void my_execve(const char *file, char *argv[], char *envp[]) {
    ...
    int fd = open(file, O_RDONLY);  //先打开这个文件
    assert(fd > 0);
    ...
    ); // 这个是一个汇编代码，这个代码的作用是把rsp的值赋值给rsp寄存器，然后跳转到e_entry的地址
}

void *init_proc_stack(char *argv[], char *envp[]) {// ABI 规定了进程栈的布局和寄存器初始状态 （这件事情GPT竟然好像回答错了，是否包含栈布局）
