...
};  // very cool OOP-like codes in C, learning them further
...
void main() {
    ...
    while (getcmd(buf, sizeof(buf)) >= 0) {
        if (buf[0] == 'c' && buf[1] == 'd' && buf[2] == ' ') {
            ...
            continue;
        }  // cd is different from others, for cd do not open a new proc but change the state of the ongoing shell
        if (syscall(SYS_fork) == 0)
            runcmd(parsecmd(buf));   // parsecmd: make a syntax tree (a interpreter)
        syscall(SYS_wait4, -1, 0, 0, 0);
    }
}
